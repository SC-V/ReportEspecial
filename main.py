...
def get_report(option="Today", start_=None, end_=None) -> pandas.DataFrame:
    ...
    for secret in CLAIM_SECRETS:
        ...
        for claim in claims:
            ...
            report_return_comment = "Return comments are disabled"
            try:
    ...
    result_frame = pandas.DataFrame(report,
                                    columns=["cutoff", "created_time", "client_id", "barcode", "claim_id", "lo_code", "status", "status_time",
                                             "pod_point_id", "pickup_address", "receiver_address", "client_comment", "courier_name", "courier_park",
...
st.markdown(f"# ~~Chile~~ Peru backup warehouse routes report")
